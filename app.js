...
function getWeekRange() {
  return {
    start: ymd(state.currentDate),
    end: ymd(addDays(state.currentDate, 6)),
  };
}
...
function sanitizeWeekResp(resp) {
  ...
  return { ok, trips, assignments, error: resp?.error };
}
...
async function withRetry(
  fn,
  { tries = 3, baseDelayMs = 350, maxDelayMs = 2000, jitter = 0.25, shouldRetry = (err) => true } = {},
) {
  let lastErr;
...
const api = {
  ...
  weekData(start, end) {
    return withRetry(
      async () => {
        return await fetchAPI("weekData", { start, end });
      },
      {
        tries: 3,
        shouldRetry: (err) => {
          ...
          return /timeout|network/i.test(String(err?.message || ""));
        },
      },
    );
  },
...
async function fetchWeekDataCached(start, end, force = false) {
  ...
  const p = (async () => {
    try {
      const raw = await api.weekData(start, end);
...
function prefetchAdjacentWeeks() {
  ...
  _prefetchTimer = setTimeout(() => {
    ...
    for (let w = -RADIUS_WEEKS; w <= RADIUS_WEEKS; w++) {
      ...
      const start = ymd(addDays(base, w * 7));
      const end = ymd(addDays(base, w * 7 + 6));

      fetchWeekDataCached(start, end).catch(() => {});
    }
  }, 120);
}
...
function setLeftCardMode(mode) {
  ...
  dom.driverWeekCard.classList.toggle("is-hidden", showTrip);

  scheduleAgendaReflow();
  if (mode === "drivers") updateDriverWeekIfVisible();
}
...
async function loadTripsForWeek(reqId) {
  ...
  const resp = await fetchWeekDataCached(start, end, true);
...
async function loadDriversAndBuses(forceRefresh = false) {
  ...
  const [driversResp, busesResp] = await Promise.all([
    state.driversList.length ? Promise.resolve({ ok: true, drivers: state.driversList }) : api.listDrivers(true),
    api.listBuses(true),
  ]);
